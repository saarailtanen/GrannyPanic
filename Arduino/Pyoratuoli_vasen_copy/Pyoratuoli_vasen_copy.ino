...
pinMode(vasenAnturi_PIN, INPUT_PULLUP);
attachInterrupt(digitalPinToInterrupt(vasenAnturi_PIN), countPulse, FALLING); // Trigger on falling edge
lastTime = millis();s
  ...
  while(true)
  {
    ...
    if ( kuuntelija.connected() && sokettiServeri.handshake(kuuntelija))
    {
      ...
      while(kuuntelija.connected())
      {
        ...
        jsondokkari["vasenAnturi1"] = String(new_rpm1);
